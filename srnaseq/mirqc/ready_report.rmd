...
root_other_path = "~/orch/scratch/simulator/mirqc/bcbio/non_mirqc_bcbio/work/report"
...
```{r setup, message="FALSE",warning=FALSE, results='hide', eval=FALSE}
...
obj_other <- IsomirDataSeqFromFiles(files = mi_files, design = design_other ,header = T, cov = 1)
...
kable(isoSelect(obj, mirna="hsa-let-7a-5p", minc=1)[,grepl("MS", rownames(design_other))])

kable(isoSelect(obj, mirna="hsa-let-7b-5p", minc=1)[,grepl("MS", rownames(design_other))])

kable(isoSelect(obj, mirna="hsa-let-7c-5p", minc=1)[,grepl("MS", rownames(design_other))])

kable(isoSelect(obj, mirna="hsa-let-7d-5p", minc=1)[,grepl("MS", rownames(design_other))])

mirnas = c("hsa-let-7a-5p", "hsa-let-7b-5p", "hsa-let-7c-5p", "hsa-let-7d-5p")
ma = log2(counts(obj)[mirnas, grepl("MS", rownames(design_other))] + 0.25)
pheatmap(ma)
```

```{r detection-serum-liver, results="asis"}

kable(isoSelect(obj, mirna="hsa-miR-302a-3p", minc=1)[,grepl("liver", rownames(design_other))])

kable(isoSelect(obj, mirna="hsa-miR-302b-3p", minc=1)[,grepl("liver", rownames(design_other))])

kable(isoSelect(obj, mirna="hsa-miR-302c-3p", minc=1)[,grepl("liver", rownames(design_other))])

kable(isoSelect(obj, mirna="hsa-miR-302d-3p", minc=1)[,grepl("liver", rownames(design_other))])

mirnas = c("hsa-miR-302a-3p", "hsa-miR-302b-3p", "hsa-miR-302c-3p", "hsa-miR-302d-3p")
ma = log2(counts(obj)[mirnas, grepl("liver", rownames(design_other))] + 0.25)
pheatmap(ma)
```
...
```{r show-mir302}
isoSelect(obj_other, "hsa-miR-302a-3p", minc=1)[, grepl("serum", rownames(design_other))]
isoSelect(obj_other, "hsa-miR-133a-3p", minc=1)[, grepl("serum", rownames(design_other))]
```
